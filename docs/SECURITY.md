...
### ID トークンと `APP_ID_SECRET`

本リポジトリではセッション外で使用する「ID トークン」を不可逆化するために、HMAC ベースのシークレット `APP_ID_SECRET` を使用します。これは可逆的な暗号ではなく、ID の署名付きトークン（検証可能だが復号不可）を生成することで実装されます。

推奨事項:
- シークレットは少なくとも **32 バイト（256 ビット）** のランダムな値を使ってください。
- シークレットはレポジトリにコミットしないでください。環境変数やシークレットストア（systemd, Docker secrets, Kubernetes Secret, Vault など）で管理してください。

生成コマンド例（bash）:

1) URL 安全な base64 形式（推奨） — 256 ビット:

```bash
openssl rand -base64 32 | tr '+/' '-_' | tr -d '='
```

2) 16 進表現（256 ビット、64 文字）:
...
ローテーションについて:
- 本実装では単一の `APP_ID_SECRET` を使用します。ローテーションする場合は新しいシークレットに切り替えると、既存のトークンは無効になります（可逆でなく署名ベースのため）。
- 無停止でローテーションしたい場合は、アプリ側で「複数のシークレットを受け入れる」仕組み（現在の秘密と直近の旧シークレット）を追加してください。将来的にはトークンにバージョンプレフィックスを付ける実装を推奨します。

注意点:
- シークレットは短くても良いが、十分なエントロピー（ランダム性）を持つことが重要です。推測可能な値（パスワードやアプリ名など）は使わないでください。
- 本番環境ではシークレットを OS のシークレットマネージャ（Vault 等）へ保存し、アプリは環境変数またはマウントされたファイルで読み込む運用を推奨します。
    deny all;
    return 404;
}
