...
try {
    ...
    /*
    if ($tag) {
        $sql .= " LEFT JOIN illust_tags it2 ON i.id = it2.illust_id
    ...
    */
    ...
    // 総数を取得
    $countSql = "SELECT COUNT(i.id) as total FROM illusts i";
    /*
    if ($tag) {
        $countSql .= " LEFT JOIN illust_tags it ON i.id = it.illust_id
